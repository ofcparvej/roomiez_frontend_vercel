      ...
      <div className = ' z-10 sticky top-0 left-0  border h-[55px]   md:h-[90px] bg-slate-400 flex flex-row  md:space-x-[150px] justify-center  w-screen item ' >
        <div className='' >
            <div className='     top-0    flex items-center shadow-lg h-[90px] md:h-[90px] bg-slate-200  z-10 sticky   border  flex-row  justify-between w-screen    ' >   
              ...
              { (Type != "Student" )? (  <div className='relative'> <button className='relative text-md  text-gray-400 h-8  text-center  rounded-md hover:text-slate-500' onClick={()=>{navigate(`/addlocations/${collegeCode}`)}}>
              ...
              {<div  className='relative'   onClick={()=>{  if(accType=='Admin'){
                navigate("/admin")
              }else{
                 navigate("/home")
              ...
              {/* <div className='relative'> <input className=' relative h-8  text-center hover:shadow-md rounded-md ' placeholder='Search' onChange={(e) => {setSearchText(e.target.value.replace(/\s+/g, ''))}}  ></input> </div> */}
                            {<div className='relative'   onClick={ ()=>{   navigate("/"); dispatch(logOutUser()) ;console.log("clicked");  }  }> <button className='relative text-md  text-gray-400 h-8  text-center  rounded-md hover:text-slate-500' >
